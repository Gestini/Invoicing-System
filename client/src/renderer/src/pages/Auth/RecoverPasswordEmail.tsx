...
const Login = () => {
    ...
    const handleSendToken = async (e) => {
        ...
        if (Object.keys(errors).length === 0 && email.length > 0) {
            if (waitTime > 0) {
                setErrors({ email: `Por favor, espera ${formatTime(waitTime / 1000)} antes de intentar nuevamente.` });
                return;
            }
    ...
    return (
        <AuthBody onClick={handleSendToken}>
            <AuthHeader title='Ingresa tu email' description='Ingresa tu email para restablecer tu contraseña.' />
            ...
            <p className='text-sm text-gray-400'>
                Si estás registrado, recibirás un correo electrónico.
            </p>
            <div className='flex justify-between'>
                <Link to='/login' className=' underline flex mt-4 text-blue-500'>Inicia sesión</Link>
                <Link to='/recovery-password/code' className=' underline flex mt-4 text-blue-500'>¿Ya recibiste el token?</Link>
            </div>
        </AuthBody>
    );
};
